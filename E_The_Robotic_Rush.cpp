...
ll helper(int ls, int rs, int val, set<ll> &st, map<ll, ll> &mp) {
    int ans = INT_MAX;
    if (ls != -1 and rs != -1) {
        int x = ls - val;
        int y = rs - val;
        int l = st.count(x), r = st.count(y);
        if (l != 0 and r != 0) {
            ans = min(mp[x], mp[y]);
        } else if (l != 0) {
            ans = mp[x];
...
vl func(vl &a, vl &b, set<ll> &st, map<ll, ll> &pos, ll &k) {
    vl die_iter(k, 0);
    for (int i{0}; i<a.size(); i++) {
        auto temp = upper_bound(all(b), a[i]);
        int ls = -1, rs = -1;
        if (temp == b.end()) {
            --temp;
            ls = *temp;
        ...
        int idx = helper(ls, rs, a[i], st, pos);
        if (idx == INT_MAX) {
            continue;
        }
        die_iter[idx]++;
    ...
    debug(die_iter);
    fn(i, 1, k) {
        die_iter[i] += die_iter[i - 1];
    }
    int val = a.size();
    vl ans;
    fl(i, k) {
        ans.push_back(max(0LL, val - die_iter[i]));
    }
    return ans;
}
