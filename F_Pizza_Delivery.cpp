...
ll dp(ll i, ll state) {
    ...
    pair<ll,ll> cur =
        (state == 0 ? highp[i] : lowp[i]);

    ll cost_high =
        dist(cur, lowp[i+1]) +
        dist(lowp[i+1], highp[i+1]) +
        dp(i+1, 0);

    ll cost_low =
        dist(cur, highp[i+1]) +
        dist(highp[i+1], lowp[i+1]) +
        dp(i+1, 1);

    return res = min(cost_high, cost_low);
}
...
void solve() {
    ...
    for (auto &k : mp) {
        ...
        lowp.push_back({k.first, mn});
        highp.push_back({k.first, mx});
    }

    m = (ll)lowp.size() - 2; 
    memo.assign(m + 2, vector<ll>(2, -1));
